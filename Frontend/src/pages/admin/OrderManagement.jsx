...
const RESTOCK_TABS = ['All', 'Pending', 'Approved', 'Rejected'];
...
const OrderManagement = () => {
    ...
    const [activeTab, setActiveTab] = useState('orders');     // 'orders' | 'returns' | 'supplierOrders'
    ...
    const [isLoadingSupplierOrders, setIsLoadingSupplierOrders] = useState(true);
    const [supplierOrderTab, setSupplierOrderTab] = useState('All');
    ...
    // ─── Fetch Supplier Orders (all restock requests) ─────────────
    const fetchSupplierOrders = useCallback(async () => {
        try {
            setIsLoadingSupplierOrders(true);
            const res = await fetch(`${API}/restock`, {
                headers: { Authorization: `Bearer ${token()}` },
            });
            const json = await res.json();
            if (json.success) setSupplierOrders(json.data);
        } catch (err) {
            console.error('fetchSupplierOrders error:', err);
        } finally {
            setIsLoadingSupplierOrders(false);
        }
    }, []);
    ...
    const handleRestockAction = async (req, action) => {
        ...
        if (!isAccept) {
            const confirm = await Swal.fire({
                ...
                html: `<span style="color:rgba(255,255,255,0.7)">You are about to decline the restock request for <strong style="color:#f1f5f9">${req.product_name}</strong>.</span>`,
                icon: 'warning',
        ...
        setProcessingRestockId(req.request_id);
        try {
            ...
            Swal.fire({
                ...
                title: isAccept ? 'Request Approved!' : 'Request Declined',
                text: isAccept
                    ? `Restock request for "${req.product_name}" has been approved.`
            ...
            setProcessingRestockId(null);
        }
    };

    // ─── Supplier Orders Filters ──────────────────────────────────
    const filteredSupplierOrders = supplierOrders.filter(r => {
        const matchTab    = supplierOrderTab === 'All' || r.status === supplierOrderTab;
        const term        = supplierSearchTerm.toLowerCase();
        const matchSearch = !term || [r.product_name, r.product_category, r.staff_name, r.supplier_name, String(r.request_id)]
            .some(v => v && v.toLowerCase().includes(term));
        return matchTab && matchSearch;
    });

    const pendingRestockCount = supplierOrders.filter(r => r.status === 'Pending').length;
    ...
    return (
        <div className="order-management">
            ...
            <div className="tab-nav">
                ...
                <button
                    className={`tab-btn ${activeTab === 'supplierOrders' ? 'tab-active' : ''}`}
                    onClick={() => setActiveTab('supplierOrders')}
                >
            ...
            {activeTab === 'supplierOrders' && (
                <>
                    ...
                    <div className="return-stats-row">
                        {[
                            { label: 'Total Requests',  value: supplierOrders.length,                                   color: '#4ecdc4', Icon: ClipboardList },
                            { label: 'Pending',         value: supplierOrders.filter(r => r.status === 'Pending').length,  color: '#f59e0b', Icon: Hourglass    },
                            { label: 'Approved',        value: supplierOrders.filter(r => r.status === 'Approved').length, color: '#10b981', Icon: CheckCircle  },
                            { label: 'Rejected',        value: supplierOrders.filter(r => r.status === 'Rejected').length, color: '#ef4444', Icon: XCircle      },
                        ].map(stat => (
                            <div key={stat.label} className="ret-stat-card">
                                <div className="ret-stat-icon" style={{ background: `${stat.color}1a`, color: stat.color }}>
                                    <stat.Icon size={18} />
                    ...
                    <div className="so-toolbar">
                        <div className="sor-tabs">
                            {RESTOCK_TABS.map(tab => {
                                const count = tab === 'All' ? supplierOrders.length : supplierOrders.filter(r => r.status === tab).length;
                                return (
                                    <button
                                        key={tab}
                                        className={`sor-tab ${supplierOrderTab === tab ? 'active' : ''}`}
                                        onClick={() => setSupplierOrderTab(tab)}
                                    >
                                        {tab}
                        ...
                        <div className="so-search">
                            ...
                            <input
                                type="text"
                                placeholder="Search product, supplier, staff…"
                                value={supplierSearchTerm}
                                onChange={e => setSupplierSearchTerm(e.target.value)}
                            />
                        </div>
                        <button className="so-refresh-btn" onClick={fetchSupplierOrders} title="Refresh">
                            <RefreshCw size={15} /> Refresh
                        </button>
                    </div>

                    {/* Cards */}
                    {isLoadingSupplierOrders ? (
                        <div style={{ textAlign: 'center', padding: '3rem', color: '#9ca3af' }}>
                            Loading supplier orders…
                        </div>
                    ) : filteredSupplierOrders.length === 0 ? (
                        <div className="so-empty">
                            <ClipboardList size={52} style={{ color: '#4ecdc4', opacity: 0.4 }} />
                            <p>{supplierSearchTerm ? 'No requests match your search.' : 'No requests found for this filter.'}</p>
                        </div>
                    ) : (
                        <div className="so-cards">
                            {filteredSupplierOrders.map(req => {
                                const s          = RESTOCK_STATUS_CONFIG[req.status] || RESTOCK_STATUS_CONFIG.Pending;
                                const StatusIcon = s.Icon;
                                const isPending  = req.status === 'Pending';
                                const isProcessing = processingRestockId === req.request_id;

                                return (
                                    <div key={req.request_id} className={`so-card ${isPending ? 'so-card-pending' : ''}`}>
                                        {/* Card Header */}
                                        <div className="so-card-header">
                                            <div className="so-card-left">
                                                ...
                                                <div>
                                                    <div className="so-product-name">{req.product_name}</div>
                                                    <div className="so-product-meta">
                                                        <span className="so-category">{req.product_category}</span>
                                                        <span className="so-req-id">REQ-{String(req.request_id).padStart(4, '0')}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <span className="sor-status-badge" style={{ color: s.color, background: s.bg, border: `1px solid ${s.border}` }}>
                                                <StatusIcon size={13} />
                                                {s.label}
                                            </span>
                                        </div>
                                        ...
                                        <div className="so-details-grid">
                                            <div className="so-detail-item">
                                                <span className="so-detail-label"><Layers size={11} /> Qty Requested</span>
                                            ...
                                            <div className="so-detail-item">
                                                <span className="so-detail-label"><Package size={11} /> Current Stock</span>
                                                <span className="so-detail-value" style={{ color: req.current_stock === 0 ? '#ef4444' : req.current_stock <= 5 ? '#f59e0b' : '#f1f5f9' }}>
                                                    {req.current_stock} units
                                                </span>
                                            </div>
                                            <div className="so-detail-item">
                                                <span className="so-detail-label"><User size={11} /> Supplier</span>
                                                <span className="so-detail-value">{req.supplier_name || '—'}</span>
                                            </div>
                                            <div className="so-detail-item">
                                                <span className="so-detail-label"><User size={11} /> Requested By</span>
                                                <span className="so-detail-value">{req.staff_name || '—'}</span>
                                            </div>
                                            <div className="so-detail-item">
                                                <span className="so-detail-label"><Clock size={11} /> Requested On</span>
                                                <span className="so-detail-value">{fmtDate(req.requested_at)}</span>
                                            </div>
                                            <div className="so-detail-item">
                                                <span className="so-detail-label"><CalendarDays size={11} /> Expected By</span>
                                                <span className="so-detail-value">{req.expected_date ? fmtDate(req.expected_date) : 'Not specified'}</span>
                                            </div>
                                            {req.notes && (
                                                <div className="so-detail-item" style={{ gridColumn: '1 / -1' }}>
                                                    <span className="so-detail-label"><FileText size={11} /> Notes</span>
                                                    <span className="so-detail-value" style={{ fontStyle: 'italic', color: 'var(--text-muted)', fontWeight: 400 }}>"{req.notes}"</span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        {isPending && (
                                            <div className="so-actions">
                                                <button
                                                    className="so-btn-decline"
                                                    onClick={() => handleRestockAction(req, 'reject')}
                                                    disabled={isProcessing}
                                                >
                                                    {isProcessing ? <span className="so-spinner-sm" /> : <X size={15} />}
                                                    Decline
                                                </button>
                                                <button
                                                    className="so-btn-accept"
                                                    onClick={() => handleRestockAction(req, 'accept')}
                                                    disabled={isProcessing}
                                                >
                                                    {isProcessing
                                                        ? <span className="so-spinner-sm" style={{ borderTopColor: '#000', borderColor: 'rgba(0,0,0,0.2)' }} />
                                                        : <Check size={15} />
                                                    }
                                                    Approve
                                                </button>
                                        ...
                                        {!isPending && (
                                            <div className="so-resolved-banner" style={{ background: s.bg, borderColor: s.border, color: s.color }}>
                                                <StatusIcon size={14} />
                                                This request was <strong style={{ marginLeft: 4 }}>{s.label}</strong>
                                                {req.updated_at && <span style={{ marginLeft: 4 }}>on {fmtDate(req.updated_at)}</span>}
                                            </div>
                                        )}
                                    </div>
                                );
            ...
            <style>{`
                ...
                .so-search {
                    ...
                    color: var(--text-muted); flex: 1; max-width: 340px;
                }
                .so-search input {
                ...
                .so-refresh-btn:hover { background: rgba(255,255,255,0.1); color: var(--text-main); }
                ...
                .so-cards { display: flex; flex-direction: column; gap: 1rem; }

                .so-card {
                    background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08);
                    border-radius: 1rem; padding: 1.5rem; transition: all 0.2s;
                }
                .so-card:hover { background: rgba(255,255,255,0.045); }
                .so-card-pending { border-left: 3px solid #f59e0b; }

                .so-card-header {
                    display: flex; align-items: flex-start; justify-content: space-between;
                    gap: 1rem; margin-bottom: 1.25rem;
                }
                .so-card-left { display: flex; align-items: center; gap: 0.875rem; }
                .so-product-thumb {
                ...
                .so-product-name { font-size: 1.05rem; font-weight: 700; color: var(--text-main); margin-bottom: 0.3rem; }
                .so-product-meta { display: flex; align-items: center; gap: 0.5rem; }
                .so-category {
                    background: rgba(255,255,255,0.06); color: var(--text-muted);
                    padding: 2px 8px; border-radius: 50px; font-size: 0.78rem;
                }
                .so-req-id { font-family: monospace; font-size: 0.78rem; color: #4ecdc4; }
                .sor-status-badge {
                ...
                .so-detail-item { display: flex; flex-direction: column; gap: 3px; }
                .so-detail-label {
                    display: flex; align-items: center; gap: 0.3rem;
                    font-size: 0.72rem; font-weight: 600; color: var(--text-muted);
                    text-transform: uppercase; letter-spacing: 0.04em;
                }
                .so-detail-value { font-size: 0.9rem; font-weight: 600; color: var(--text-main); }

                .so-actions {
                    display: flex; justify-content: flex-end; gap: 0.75rem; padding-top: 0.25rem;
                }
                .so-btn-decline, .so-btn-accept {
                    display: flex; align-items: center; gap: 0.4rem;
                ...
                .so-btn-decline:hover:not(:disabled) { background: rgba(239,68,68,0.1); }
                .so-btn-accept {
                    background: linear-gradient(135deg, #4ecdc4, #44a08d); border: none; color: #000;
                }
                .so-btn-accept:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 4px 14px rgba(78,205,196,0.35); }
                .so-btn-decline:disabled, .so-btn-accept:disabled { opacity: 0.5; cursor: not-allowed; transform: none; }
                .so-spinner-sm {
                ...
                @media (max-width: 640px) {
                    ...
                    .so-details-grid { grid-template-columns: 1fr 1fr; }
                }
            `}</style>
        </div>
    );
